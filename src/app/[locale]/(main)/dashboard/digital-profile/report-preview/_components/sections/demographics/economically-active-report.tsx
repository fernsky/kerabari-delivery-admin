...
export function EconomicallyActiveReport() {
  ...
  const processedData: ProcessedEconomicallyActiveData | null = useMemo(() => {
    ...
    return processEconomicallyActiveData(mappedData);
  }, [rawData]);
  ...
  const charts = useMemo(() => {
    ...
    Object.entries(processedData.wardData).forEach(([wardNum, data]) => {
      ...
      Object.entries(data.ageGroups).forEach(([ageGroup, population]) => {
        const label = processedData.ageGroupData[ageGroup]?.label || ageGroup;
  ...
  return (
    <div className="section-content" id="section-economically-active">
      ...
      <div className="table-section">
        <h3 className="table-title">तालिका ३.१६.१: उमेर समूह अनुसार आर्थिक रूपले सक्रिय जनसंख्या विस्तृत विवरण</h3>
        <table className="data-table economically-active-table">
          ...
          <tbody>
            ...
            <tr className="total-row">
              ...
              <td className="grand-total-cell">{convertToNepaliNumber(processedData.totalEconomicallyActivePopulation)}</td>
      ...
      {/* Bar Chart */}
      <div className="chart-section">
        <h3 className="chart-title">चित्र ३.१६.२: वडागत आर्थिक रूपले सक्रिय जनसंख्या वितरण</h3>
        <div className="pdf-chart-container">
          <div 
            style={{ 
              width: "100%", 
      ...
      <div className="table-section">
        <h3 className="table-title">तालिका ३.१६.२: वडा अनुसार आर्थिक रूपले सक्रिय जनसंख्या विवरण</h3>
        <table className="data-table ward-economically-active-table">
          <thead>
            <tr>
              <th>वडा नं.</th>
              <th>प्रमुख उमेर समूह</th>
              <th>प्रमुख उमेर समूहको जनसंख्या</th>
              <th>प्रमुख उमेर समूहको प्रतिशत</th>
              <th>कुल आर्थिक रूपले सक्रिय जनसंख्या</th>
            </tr>
          </thead>
          <tbody>
            {Object.entries(processedData.wardData)
              ...
              .map(([wardNum, data], index) => (
                <tr key={wardNum}>
                  ...
                  <td>{convertToNepaliNumber(data.ageGroups[data.primaryAgeGroup] || 0)}</td>
                  <td>{formatNepaliPercentage(data.primaryAgeGroupPercentage)}</td>
                  <td>{convertToNepaliNumber(data.totalEconomicallyActivePopulation)}</td>
                </tr>
              ))}
          </tbody>
        </table>
